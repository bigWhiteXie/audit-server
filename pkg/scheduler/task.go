...
type ScheduleTask struct {
	...
	TaskName       string    `gorm:"uniqueIndex:idx_task_name" json:"task_name"` // 任务唯一标识，唯一索引
	LeaseHolder    string    `json:"lease_holder"`                               // 当前持有者（实例ID）
	LeaseUntil     time.Time `json:"lease_until"`                                // 租约到期时间
	LastRunTime    time.Time `json:"last_runtime"`                               // 上次执行时间
	NextRunTime    time.Time `json:"next_runtime"`                               // 理论下次执行时间
